...
- 客户端脚本上传到路由器后只需手动运行一次：第一次运行时脚本会自动检测并写入 crontab 定时任务，不需要再单独执行 `crontab -e`
...
1. **下载并替换文件**——确认 `geoip.dat`、`geosite.dat` 已经生成，且服务已重启
2. **自动写入定时任务**——脚本自己检测 crontab 里有没有写过自己这一行，没有就自动加上，日志里会看到类似「首次运行：已自动写入定时任务（`0 1 * * 1`，路由器本地时间），以后不用再手动 `crontab -e` 了」

也就是说，**这一步做完，部署就已经全部完成了**，不需要再单独进入 crontab 手动加一行。可以用下面命令确认已经写进去：

```sh
crontab -l
```

应该能看到类似这一行（`sh` 后面跟的是脚本实际所在的完整路径）：

```
0 1 * * 1 sh /root/update-geodata-client-cf.sh
```

默认调度频率是**每周一凌晨 1 点**（按路由器系统本地时间）。如果想改成别的频率，改脚本开头这一行配置后重新运行一次即可，脚本会自动检测原来那一行还在不在、按新配置重新写：

```sh
CRON_SCHEDULE="0 1 * * 1"   # 分 时 日 月 周，语法同 crontab
```

> 这一行按路由器系统本地时间执行——如果路由器时区已经是 `Asia/Shanghai`（GMT+8，可用 `date` 命令确认），默认值就是最终结果，不需要再做任何换算。只有当路由器系统时区是 UTC 时才需要换算成 UTC 时间填写。

如果不想要这个自动安装行为（比如你自己已经用别的方式管理 crontab），把脚本开头的 `AUTO_INSTALL_CRON=1` 改成 `0` 即可，脚本会跳过这一步，只做文件更新。
...
这次应该看到日志显示"均为最新，无需下载"，同时**不会再重复写入 crontab**（脚本检测到自己已经在里面了，会跳过）。如果文件又重新下载了，说明两边计算出的 sha256 没对上，需要排查（比如 Worker 那边是不是刚好又跑了一次定时任务产生了新版本）。

至此定时任务已经在跑了，不需要再手动做任何事。如果确实想手动确认 cron 服务本身状态正常，可以：
